...
def load_context_config(_project_root: Path | None = None) -> dict[str, Any]:
    """Load ``context.yaml`` from the framework package.

    ``_project_root`` is accepted for backward compatibility but ignored;
    the config always comes from the installed framework package.
    """
    config_path = framework_root() / "context.yaml"
