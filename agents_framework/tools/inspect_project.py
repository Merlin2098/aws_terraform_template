...
from agents_framework.runtime.config import (
    entrypoint_roots,
    ignore_dirs,
...
def _detect_languages(project_root: Path, files: list[Path]) -> dict[str, Any]:
    ...
    for candidate in ("python", "typescript", "sql", "terraform"):
        if counts[candidate]:
...
def _detect_data_stack(project_root: Path, files: list[Path]) -> dict[str, Any]:
    ...
    for path in files:
        ...
        if path.name != "pyproject.toml" and path.suffix.lower() not in {".py", ".ts"}:
            continue
...
def _detect_cloud(project_root: Path, files: list[Path]) -> dict[str, Any]:
    ...
    for path in files:
        suffix = path.suffix.lower()
        if suffix not in {".py", ".tf", ".yaml", ".yml", ".json", ".ts"}:
