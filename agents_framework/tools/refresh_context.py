...
from agents_framework.hooks.treemap import write_treemap
from agents_framework.runtime.config import artifact_paths, load_context_config
from agents_framework.runtime.context_bundle import build_and_persist_context_bundle
from agents_framework.runtime.dependency_graph import build_and_persist_dependency_graph
from agents_framework.runtime.skill_registry import (
    build_and_persist_skills_registry,
    build_skills_registry,
)
from agents_framework.tools.inspect_project import inspect_project
...
def refresh_context(project_root: Path) -> dict[str, object]:
    ...
    if resolved.profile.source not in {"yaml", "yaml-legacy"} and not active_artifacts:
        active_artifacts = {
            "skills_registry",
            "context_bundle",
            "dependency_graph",
