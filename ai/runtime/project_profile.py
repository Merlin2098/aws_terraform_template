...
from ai.runtime.capability_registry import CapabilityDescriptor, load_registry
from ai.runtime.profile import Profile, load_profile
...
def _project_dependencies(project_root: Path) -> tuple[set[str], set[str]]:
    pyproject = project_root / "pyproject.toml"
    if not pyproject.exists():
        raise ValueError("pyproject.toml is required to resolve dependencies.")
    with pyproject.open("rb") as handle:
        loaded = tomllib.load(handle)
    project = loaded.get("project") or {}
    extras = set((project.get("optional-dependencies") or {}).keys())
    groups = set((loaded.get("dependency-groups") or {}).keys())
...
def resolve_project_profile(
    ...
    *,
    ...
    if validate_dependencies:
        available_extras, available_groups = _project_dependencies(project_root)
        missing_extras = sorted(set(extras) - available_extras)
        missing_groups = sorted(set(groups) - available_groups)
        if missing_extras or missing_groups:
            messages = []
            if missing_extras:
                messages.append(
